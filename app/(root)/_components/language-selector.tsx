...
export default function LanguageSelector({ mode }: { mode: boolean }) {
  ...
  const handleLanguageSelect = (langId: string) => {
    if (!mode && langId !== "javascript") return;
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <motion.button
        ...
        className={`group relative flex items-center gap-3 rounded-lg border border-gray-800/50 bg-[#1e1e2e]/80 px-4 py-2.5 transition-all duration-200 hover:border-gray-700 ${!mode && language !== "javascript" ? "cursor-not-allowed opacity-50" : ""}`}
      >
        {/* Decoration */}
        <div
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute left-0 top-full z-50 mt-2 w-64 rounded-xl border border-[#313244] bg-[#1e1e2e]/95 py-2 shadow-2xl backdrop-blur-xl"
          >
